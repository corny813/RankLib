...
public class ExpressionEvaluator {
	...
	private Queue toPostFix(String expression) {
		...
		for (int i = 0; i < expression.length(); i++) {
			...
			if (token.compareTo("(") == 0)
				...
				if (!foundOpen) {
					System.err.println("Error: Invalid expression: \"" + expression + "\". Parentheses mismatched.");
				}
			} else if (isOperator(token)) {
				if (lastReadToken.compareTo("(") == 0 || isOperator(lastReadToken))// @token
																					// is
																					// a
																					// unary
																					// opeartor
																					...
																					// -3)
				{
					if (token.compareTo("-") == 0)// convert it to a neg
													// function
						op.push("neg");
					...
					if (op.size() > 0) {
						...
						if (last.compareTo("(") == 0)
							...
							op.push(last);// push the last operator back into
											// the stack
						else if (priority.get(token) < priority.get(last))
							output.enqueue(last);
						else // equal priority
						{
							if (token.compareTo("^") == 0)// right-associative
															// operator
								op.push(last);// push the last operator back
												// into the stack
							else
								output.enqueue(last);
						}
			...
			} else // maybe function, maybe operand
			{
				...
				if (isFunction(token)) {
					if (j == expression.length()) {
						System.err.println("Error: Invalid expression: \"" + expression
								+ "\". Function specification requires parentheses.");
					}
				...
				} else// operand
				{
					try {
						...
						System.err.println("Error: \"" + token + "\" is not a valid token.");
					}
	...
	public double eval(String expression) {
		...
		try {
			while (output.size() > 0) {
				...
				if (isOperator(token)) {
					...
					if (token.compareTo("log") == 0) {
						if (eval[cp - 1] < 0) {
							System.err.println("Error: expression " + expression
									+ " involves taking log of a non-positive number");
						}
						...
						if (eval[cp - 1] < 0) {
							System.err.println("Error: expression " + expression
									+ " involves taking square root of a negative number");
						}
			...
			System.err.println("Unknown error in ExpressionEvaluator::eval() with \"" + expression + "\"");
		}
		return eval[cp - 1];
	}
}
