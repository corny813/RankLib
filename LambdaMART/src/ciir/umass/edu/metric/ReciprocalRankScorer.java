...
public class ReciprocalRankScorer extends MetricScorer {
	...
	public double[][] swapChange(RankList rl) {
		...
		double rr = 0.0;
		// consider swapping the first rank doc with everything else further
		// down the list
		if (firstRank != -1) {
			rr = 1.0 / (firstRank + 1);
			for (int j = firstRank + 1; j < size; j++) {
				...
				{
					if (secondRank == -1 || j < secondRank)// after the swap, j
															// is now the
															// position of our
															// used-to-be
															// firstRank
		...
		// now it's time to consider swapping docs at earlier ranks than the
		// first rank with those below it (and *it* too)
